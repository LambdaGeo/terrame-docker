...
--
-- Ou um script LuccME solto (ex.: benchmark/references/lab01_md1643/lab1_main.lua), rodado a
-- partir da própria pasta; NAME dá o nome do CSV:
--
...
local lab = script and assert(os.getenv("NAME"), "com SCRIPT, defina NAME")
	or assert(os.getenv("LAB"), "defina LAB (ex.: lab01) ou SCRIPT + NAME")
...
if script then
	-- Script solto: roda na pasta dele (os caminhos relativos do script valem)
	...
	print(string.format("[harness] %s: %d arquivos da saída original em %s", lab, moved, out))
end

if csvFile then csvFile:close() end
print(string.format("[harness] %s: CSV gravado em %s", lab, out))
