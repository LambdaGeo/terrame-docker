...
Chamado por generate_goldens.sh:
...
def engine_info(image: str) -> dict:
    version = subprocess.run(["docker", "run", "--rm", image], capture_output=True, text=True).stdout
...
def main() -> None:
    ...
    manifest: dict = {
        "lab": lab,
        "source": {
            "script": f"luccme/tests/functional/{lab}.lua",
            "sha256": sha256(work / f"{lab}.script.lua"),
        },
        "engine": engine_info(image),
        "generator": "benchmark/luccme/harness.lua",
    }
    ...
    iterations = re.findall(r"allocated correctly in[^\d]*(\d{4})\.?\s*Number of iterations: (\d+)",
                            (work / f"{lab}.log").read_text(errors="replace"))
    manifest.update({
        ...
        "crosscheck_vs_original_output": {
            ...
            "note": "último ano do CSV comparado com o shapefile salvo pelo próprio lab",
        },
        "iterations_per_year": {y: int(n) for y, n in iterations},
    })
    (dest / "manifest.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n")
